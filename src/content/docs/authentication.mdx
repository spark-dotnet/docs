...
## Setup
Spark's authentication is added to the DI container for you in `AppServiceRegistration.AddAppServices()`.

```csharp
services.AddAuthentication<IAuthValidator>(config);
```

The implementation for `IAuthValidator` is the `Application.Services.Auth.AuthValidator.cs` class. This service comes with 1 method that is called whenever an authentication check needs to occur, `ValidateAsync()`.
