...
### Access Denied Route
In MVC and Razor Pages, Spark sets up an access denied route for you.

Anytime a Authorize attribute with a Role check fails, the user will be redirected to the `/access-denied` route.

### Blazor AuthorizeView
In Blazor, you can combine the `<AuthorizeView>` tag helper with Roles to display different content.

This is helpful when you want to display the page but show different content based on the User's Roles.
```razor
@page "/admin/dashboard"
@attribute [Authorize]
<section>
