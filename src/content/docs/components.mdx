...
```bash
spark make component MyComponent
```
After running this command, Spark will create 2 new files in your application.

The first will be a partial class: `Pages/Shared/MyComponent.cs`.
...
The second will be the Razor file: `Pages/Shared/MyComponent.razor`
...
You can create components in subdirectories as well. For example, the following commands will create a `PostHeader` component in the `Posts` subdirectory.
```bash
spark make component Posts/PostHeader
```

### Inline Components
If your component is fairly small, you can create an **inline** component. Inline components are a single .razor file that have an `@code` block to contain it's logic, rather than a seperate partial class file.
```razor
<div>
...
```bash
spark make component MyComponent --inline
```
