...
## Setup
Because all of your configuraiton values end up in the `appsettings.json` file, they can be accessed by the `IConfiguration` interface.

This is accessed via dependency injection.

```csharp
// class or controller example
private IConfiguration _config;

public CacheController(IConfiguration config)
{
    this._config = config;
}
```
...
```csharp
_config.GetValue<string>("Spark:App:Name")!;
```

Environment variables can also be accessed through the static `Spark.Library.Environment.Env` class.
```csharp
