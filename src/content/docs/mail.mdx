---
title: "Mail"
description: "Blazor Spark Mail"
---

# Mail
Sending email should be easy. Blazor Spark provides a clean, simple email interface powered by the popular [Mailkit](https://github.com/jstedfast/MailKit) library.

## Configuration
Blazor Spark comes with multiples drivers that change how e-mails are sent.
...
Mailables inherit from `BlazorSpark.Library.Mail.Mailable` and accept a generic type you want associated with sending your mail.
...
## Sending Mail
Inject the `IMailer` interface into your Blazor components or service and pass a **Mailable** to the `SendAsync` method:
...
## Generic Mailable
Blazor Spark comes with a generic mailable for scenarios where you don't want to create your own custom **Mailable**.
```csharp
